...
try:
    ...
    cursor.execute("""
    SELECT s.supplier_name as 'Supplier', so.expected_date as 'Expected Delivery', so.delivery_date 'Actual Delivery',
    DATEDIFF(so.delivery_date,so.expected_date) as Gap
    FROM suppliers s JOIN parts p ON s.supplier_id = p.supplier_id
    JOIN supply_order so ON so.part_id = p.part_id
    """)
    ...
    FROM employees e JOIN hours_worked h on e.employee_id = h.employee_id
